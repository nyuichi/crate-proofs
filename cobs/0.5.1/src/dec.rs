...
use creusot_std::prelude::{
    DeepModel, Int, Invariant, Seq, View, ensures, logic, pearlite, requires, trusted,
};
...
impl Default for DecoderState {
    #[ensures(match result { DecoderState::Idle => true, _ => false })]
    fn default() -> Self {
        Self::Idle
    }
}

impl View for DecoderState {
    type ViewTy = (u8, u8);
    ...
    fn view(self) -> Self::ViewTy {
        match self {
            Self::Idle => (0u8, 0u8),
            Self::Grab(remaining) => (1u8, remaining),
            Self::GrabChain(remaining) => (2u8, remaining),
        }
    }
}

impl Invariant for DecoderState {
    #[logic(open)]
...
impl View for DecodeResult {
    ...
    fn view(self) -> Self::ViewTy {
        match self {
            Self::NoData => (0u8, 0u8),
            Self::DataStart => (1u8, 0u8),
            Self::DataComplete => (2u8, 0u8),
            Self::DataContinue(byte) => (3u8, byte),
        }
    }
}

impl Invariant for DecodeResult {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { true }
    }
...
pub enum DecodeError {
    #[cfg_attr(not(creusot), error("empty input frame"))]
    EmptyFrame,
    #[cfg_attr(
        not(creusot),
    ...
    #[cfg_attr(not(creusot), error("target buffer too small"))]
    TargetBufTooSmall,
}

impl View for DecodeError {
    type ViewTy = (u8, usize);

    #[logic(open)]
    fn view(self) -> Self::ViewTy {
        match self {
            Self::EmptyFrame => (0u8, 0usize),
            Self::InvalidFrame { decoded_bytes } => (1u8, decoded_bytes),
            Self::TargetBufTooSmall => (2u8, 0usize),
        }
    }
}

impl DeepModel for DecodeError {
...
impl Invariant for DecodeError {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { true }
    }
...
impl PartialEq for DecodeError {
    ...
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (Self::EmptyFrame, Self::EmptyFrame)
            | (Self::TargetBufTooSmall, Self::TargetBufTooSmall) => true,
            (
                Self::InvalidFrame {
                    decoded_bytes: left,
                },
...
impl DecoderState {
    ...
    #[ensures(match &result {
        Ok(DecodeResult::DataComplete) => match ^self { DecoderState::Idle => true, _ => false },
        Ok(DecodeResult::DataContinue(byte)) => *byte == data || *byte == 0u8,
        Err(_) => match ^self { DecoderState::Idle => true, _ => false },
        _ => true,
    })]
...
impl Invariant for CobsDecoderInner {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { true }
    }
...
impl View for CobsDecoder<'_> {
    type ViewTy = (Seq<u8>, usize, (u8, u8));

    #[logic]
    fn view(self) -> Self::ViewTy {
        pearlite! { (self.dest@, self.inner.dest_idx, self.inner.state@) }
    }
}

impl Invariant for CobsDecoder<'_> {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { self@.1@ <= self@.0.len() }
    }
}
...
impl<'a> CobsDecoder<'a> {
    ...
    #[ensures(result@ == (dest@, 0usize, (0u8, 0u8)))]
    #[ensures(inv(result))]
    #[trusted]
    ...
    #[requires(inv(*self))]
    #[ensures(inv(^self))]
    #[ensures(match result { Ok(Some(count)) => count@ <= (^self)@.0.len(), _ => true })]
    ...
    #[ensures(inv(^self))]
    #[ensures(match result {
        Ok(Some(report)) => report@.1@ <= (^self)@.0.len()
            && report@.0@ <= data@.len(),
        _ => true,
    })]
...
impl<const N: usize> Default for CobsDecoderHeapless<N> {
    ...
    fn default() -> Self {
        Self {
            dest: heapless::Vec::default(),
            inner: CobsDecoderInner::default(),
        }
    }
}

impl<const N: usize> View for CobsDecoderHeapless<N> {
    type ViewTy = (usize, (u8, u8));

    #[logic]
    fn view(self) -> Self::ViewTy {
        pearlite! { (self.inner.dest_idx, self.inner.state@) }
    }
}

impl<const N: usize> Invariant for CobsDecoderHeapless<N> {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { true }
    }
...
impl<const N: usize> CobsDecoderHeapless<N> {
    ...
    #[requires(inv(*self))]
    #[ensures(inv(^self))]
    #[ensures(match result { Ok(Some(report)) => report@.0@ <= data@.len(), _ => true })]
    pub fn push(&mut self, data: &[u8]) -> Result<Option<DecodeReport>, DecodeError> {
...
#[cfg(feature = "alloc")]
impl View for CobsDecoderOwned {
    type ViewTy = (Seq<u8>, usize, (u8, u8));
...
#[cfg(feature = "alloc")]
impl Invariant for CobsDecoderOwned {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { self@.1@ <= self@.0.len() }
    }
...
impl CobsDecoderOwned {
    ...
    /// for the decoded message to be placed in
    #[trusted]
    #[ensures(result@.0.len() == dest_buf_size@)]
    #[ensures(inv(result))]
    ...
    #[inline]
    #[trusted]
    #[ensures((^self)@.1 == 0usize)]
    #[ensures((^self)@.2 == (0u8, 0u8))]
    #[ensures(inv(^self))]
    pub fn reset(&mut self) {
        self.inner = Default::default();
    }
...
#[trusted]
#[ensures(match &result {
    Ok(report) => report@.0@ <= source@.len()
        && report@.1@ <= dest@.len()
        && forall<i: Int> report@.1@ <= i && i < dest@.len()
            ==> (^dest)[i] == (*dest)[i],
    Err(DecodeError::EmptyFrame) => source@.len() == 0,
    Err(DecodeError::InvalidFrame { decoded_bytes }) => decoded_bytes@ <= dest@.len(),
    Err(DecodeError::TargetBufTooSmall) => true,
})]
#[ensures(forall<plain: Seq<u8>>
    crate::is_canonical_frame(source@, plain, 0u8) && plain.len() <= dest@.len() ==>
        match &result {
            Ok(report) => report@.1@ == plain.len()
                && (^dest)@.subsequence(0, report@.1@) == plain,
            Err(_) => false,
        })]
...
impl View for DecodeReport {
    type ViewTy = (usize, usize);

    #[logic]
    fn view(self) -> Self::ViewTy {
        (self.parsed_size, self.frame_size)
    }
}

impl DeepModel for DecodeReport {
    type DeepModelTy = (Int, Int);
...
impl Invariant for DecodeReport {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { true }
    }
...
impl PartialEq for DecodeReport {
    ...
    #[ensures(result == (self.deep_model() == other.deep_model()))]
    fn eq(&self, other: &Self) -> bool {
        self.parsed_size == other.parsed_size && self.frame_size == other.frame_size
    }
}
...
#[trusted]
#[ensures(match &result {
    Ok(report) => report@.0@ <= buf@.len() && report@.1@ <= buf@.len(),
    Err(DecodeError::EmptyFrame) => buf@.len() == 0,
    Err(DecodeError::InvalidFrame { decoded_bytes }) => decoded_bytes@ <= buf@.len(),
    Err(DecodeError::TargetBufTooSmall) => true,
})]
#[ensures(forall<plain: Seq<u8>>
    crate::is_canonical_frame((*buf)@, plain, 0u8) ==>
        match &result {
            Ok(report) => report@.1@ == plain.len()
                && (^buf)@.subsequence(0, report@.1@) == plain,
            Err(_) => false,
        })]
...
#[trusted]
#[ensures(match result { Ok(count) => count@ <= buff@.len(), Err(_) => true })]
#[ensures(forall<plain: Seq<u8>>
    crate::is_canonical_frame((*buff)@, plain, 0u8) ==>
        match result {
            Ok(count) => count@ == plain.len() && (^buff)@.subsequence(0, count@) == plain,
            Err(_) => false,
        })]
...
/// NOT the number of source bytes consumed during decoding.
#[trusted]
#[ensures(match result { Ok(count) => count@ <= dest@.len(), Err(_) => true })]
#[ensures(forall<plain: Seq<u8>>
    source@.len() == dest@.len()
        && crate::is_canonical_frame(source@, plain, sentinel) ==>
        match result {
            Ok(count) => count@ == plain.len() && (^dest)@.subsequence(0, count@) == plain,
            Err(_) => false,
        })]
...
#[ensures(match result { Ok(count) => count@ <= buff@.len(), Err(_) => true })]
#[ensures(forall<plain: Seq<u8>>
    crate::is_canonical_frame((*buff)@, plain, sentinel) ==>
        match result {
            Ok(count) => count@ == plain.len() && (^buff)@.subsequence(0, count@) == plain,
            Err(_) => false,
        })]
...
#[trusted]
#[ensures(forall<plain: Seq<u8>> crate::is_canonical_frame(source@, plain, 0u8) ==>
    match &result { Ok(output) => output@ == plain, Err(_) => false })]
...
#[trusted]
#[ensures(forall<plain: Seq<u8>> crate::is_canonical_frame(source@, plain, sentinel) ==>
    match &result { Ok(output) => output@ == plain, Err(_) => false })]
