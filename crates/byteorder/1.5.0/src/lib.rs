...
use creusot_std::prelude::{
    ensures, logic, pearlite, requires, trusted, variant, DeepModel, Int,
    Invariant, Seq,
};

use core::{convert::TryInto, fmt::Debug, hash::Hash, mem::align_of, slice};

/// Mathematical value of `256^exponent`.
#[logic(open)]
#[variant(exponent)]
pub fn pow256(exponent: Int) -> Int {
    if exponent <= 0 {
        1
    } else {
        pearlite! { 256 * pow256(exponent - 1) }
    }
}

/// Decodes the first `count` bytes as an unsigned big-endian integer.
#[logic]
#[requires(0 <= count && count <= bytes.len())]
#[variant(count)]
pub fn decode_be(bytes: Seq<u8>, count: Int) -> Int {
    if count <= 0 {
        0
    } else {
        pearlite! {
            decode_be(bytes, count - 1) * 256 + bytes[count - 1]@
        }
    }
}

/// Decodes the first `count` bytes as an unsigned little-endian integer.
#[logic]
#[requires(0 <= count && count <= bytes.len())]
#[variant(count)]
pub fn decode_le(bytes: Seq<u8>, count: Int) -> Int {
    if count <= 0 {
        0
    } else {
        pearlite! {
            decode_le(bytes, count - 1) + bytes[count - 1]@ * pow256(count - 1)
        }
    }
}

/// The byte at `index` in an integer encoding of `width` bytes.
#[logic(open)]
#[requires(0 <= index && index < width)]
pub fn encoded_byte(
    value: Int,
    width: Int,
    index: Int,
    ...
    pearlite! {
        (value / pow256(if big_endian { width - index - 1 } else { index })) % 256
    }
}

/// Signed two's-complement interpretation of an unsigned `width`-byte value.
#[logic(open)]
#[requires(width > 0)]
pub fn signed_value(unsigned: Int, width: Int) -> Int {
    if unsigned < pow256(width) / 2 {
        unsigned
    } else {
        pearlite! { unsigned - pow256(width) }
    }
}
...
pub trait ByteOrder:
    ...
    + private::Sealed
{
    /// Logical unsigned decoding model used by every runtime method contract.
    #[logic]
    #[requires(0 <= count && count <= bytes.len())]
    fn decode(bytes: Seq<u8>, count: Int) -> Int;
    ...
    #[requires(buf@.len() >= 2)]
    #[ensures(result@ == Self::decode(buf@, 2))]
    ...
    #[trusted]
    #[requires(buf@.len() >= 3)]
    #[ensures(result@ == Self::decode(buf@, 3))]
    ...
    /// ```
    #[requires(buf@.len() >= 4)]
    #[ensures(result@ == Self::decode(buf@, 4))]
    ...
    #[trusted]
    #[requires(buf@.len() >= 6)]
    #[ensures(result@ == Self::decode(buf@, 6))]
    ...
    /// ```
    #[requires(buf@.len() >= 8)]
    #[ensures(result@ == Self::decode(buf@, 8))]
    ...
    /// ```
    #[requires(buf@.len() >= 16)]
    #[ensures(result@ == Self::decode(buf@, 16))]
    ...
    /// ```
    #[requires(1 <= nbytes@ && nbytes@ <= 8 && nbytes@ <= buf@.len())]
    #[ensures(result@ == Self::decode(buf@, nbytes@))]
    ...
    /// ```
    #[requires(1 <= nbytes@ && nbytes@ <= 16 && nbytes@ <= buf@.len())]
    #[ensures(result@ == Self::decode(buf@, nbytes@))]
    ...
    /// ```
    #[requires(buf@.len() >= 2)]
    #[ensures(forall<i> 0 <= i && i < 2 ==> (^buf)[i]@ == encoded_byte(n@, 2, i, Self::decode(Seq::singleton(1u8).push_back(0u8), 2) == 256))]
    #[ensures(forall<i> 2 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    /// ```
    #[trusted]
    #[requires(buf@.len() >= 3 && n@ < pow256(3))]
    #[ensures(forall<i> 0 <= i && i < 3 ==> (^buf)[i]@ == encoded_byte(n@, 3, i, Self::decode(Seq::singleton(1u8).push_back(0u8), 2) == 256))]
    #[ensures(forall<i> 3 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    /// ```
    #[requires(buf@.len() >= 4)]
    #[ensures(forall<i> 0 <= i && i < 4 ==> (^buf)[i]@ == encoded_byte(n@, 4, i, Self::decode(Seq::singleton(1u8).push_back(0u8), 2) == 256))]
    #[ensures(forall<i> 4 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    /// ```
    #[trusted]
    #[requires(buf@.len() >= 6 && n@ < pow256(6))]
    #[ensures(forall<i> 0 <= i && i < 6 ==> (^buf)[i]@ == encoded_byte(n@, 6, i, Self::decode(Seq::singleton(1u8).push_back(0u8), 2) == 256))]
    #[ensures(forall<i> 6 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    /// ```
    #[requires(buf@.len() >= 8)]
    #[ensures(forall<i> 0 <= i && i < 8 ==> (^buf)[i]@ == encoded_byte(n@, 8, i, Self::decode(Seq::singleton(1u8).push_back(0u8), 2) == 256))]
    #[ensures(forall<i> 8 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    /// ```
    #[requires(buf@.len() >= 16)]
    #[ensures(forall<i> 0 <= i && i < 16 ==> (^buf)[i]@ == encoded_byte(n@, 16, i, Self::decode(Seq::singleton(1u8).push_back(0u8), 2) == 256))]
    #[ensures(forall<i> 16 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    /// ```
    #[requires(1 <= nbytes@ && nbytes@ <= 8 && nbytes@ <= buf@.len())]
    #[requires(n@ < pow256(nbytes@))]
    #[ensures(forall<i> 0 <= i && i < nbytes@ ==> (^buf)[i]@ == encoded_byte(n@, nbytes@, i, Self::decode(Seq::singleton(1u8).push_back(0u8), 2) == 256))]
    #[ensures(forall<i> nbytes@ <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    #[inline]
    #[requires(buf@.len() >= 2)]
    #[ensures(result@ == signed_value(Self::decode(buf@, 2), 2))]
    ...
    #[inline]
    #[requires(buf@.len() >= 3)]
    #[ensures(result@ == signed_value(Self::decode(buf@, 3), 3))]
    ...
    #[inline]
    #[requires(buf@.len() >= 4)]
    #[ensures(result@ == signed_value(Self::decode(buf@, 4), 4))]
    ...
    #[inline]
    #[requires(buf@.len() >= 6)]
    #[ensures(result@ == signed_value(Self::decode(buf@, 6), 6))]
    ...
    #[inline]
    #[requires(buf@.len() >= 8)]
    #[ensures(result@ == signed_value(Self::decode(buf@, 8), 8))]
    ...
    #[inline]
    #[requires(buf@.len() >= 16)]
    #[ensures(result@ == signed_value(Self::decode(buf@, 16), 16))]
    ...
    #[inline]
    #[requires(1 <= nbytes@ && nbytes@ <= 8 && nbytes@ <= buf@.len())]
    #[ensures(result@ == signed_value(Self::decode(buf@, nbytes@), nbytes@))]
    ...
    #[inline]
    #[requires(buf@.len() >= 2)]
    #[ensures(Self::decode((^buf)@, 2) == if n@ < 0 { n@ + pow256(2) } else { n@ })]
    #[ensures(forall<i> 2 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    #[trusted]
    #[inline]
    #[requires(buf@.len() >= 3 && -pow256(3) / 2 <= n@ && n@ < pow256(3) / 2)]
    #[ensures(Self::decode((^buf)@, 3) == if n@ < 0 { n@ + pow256(3) } else { n@ })]
    #[ensures(forall<i> 3 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    #[inline]
    #[requires(buf@.len() >= 4)]
    #[ensures(Self::decode((^buf)@, 4) == if n@ < 0 { n@ + pow256(4) } else { n@ })]
    #[ensures(forall<i> 4 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    #[trusted]
    #[inline]
    #[requires(buf@.len() >= 6 && -pow256(6) / 2 <= n@ && n@ < pow256(6) / 2)]
    #[ensures(Self::decode((^buf)@, 6) == if n@ < 0 { n@ + pow256(6) } else { n@ })]
    #[ensures(forall<i> 6 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    #[inline]
    #[requires(buf@.len() >= 8)]
    #[ensures(Self::decode((^buf)@, 8) == if n@ < 0 { n@ + pow256(8) } else { n@ })]
    #[ensures(forall<i> 8 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    #[inline]
    #[requires(buf@.len() >= 16)]
    #[ensures(Self::decode((^buf)@, 16) == if n@ < 0 { n@ + pow256(16) } else { n@ })]
    #[ensures(forall<i> 16 <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    #[inline]
    #[requires(1 <= nbytes@ && nbytes@ <= 8 && nbytes@ <= buf@.len())]
    #[requires(-pow256(nbytes@) / 2 <= n@ && n@ < pow256(nbytes@) / 2)]
    #[ensures(Self::decode((^buf)@, nbytes@) == if n@ < 0 { n@ + pow256(nbytes@) } else { n@ })]
    #[ensures(forall<i> nbytes@ <= i && i < buf@.len() ==> (^buf)[i] == (*buf)[i])]
    ...
    /// ```
    #[requires(src@.len() == dst@.len() * 2)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == Self::decode(src@.subsequence(i * 2, (i + 1) * 2), 2))]
    ...
    /// assert_eq!(numbers_given, numbers_got);
    /// ```
    #[requires(src@.len() == dst@.len() * 4)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == Self::decode(src@.subsequence(i * 4, (i + 1) * 4), 4))]
    ...
    /// assert_eq!(numbers_given, numbers_got);
    /// ```
    #[requires(src@.len() == dst@.len() * 8)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == Self::decode(src@.subsequence(i * 8, (i + 1) * 8), 8))]
    ...
    /// assert_eq!(numbers_given, numbers_got);
    /// ```
    #[requires(src@.len() == dst@.len() * 16)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == Self::decode(src@.subsequence(i * 16, (i + 1) * 16), 16))]
    ...
    #[requires(src@.len() == dst@.len() * 2)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == signed_value(Self::decode(src@.subsequence(i * 2, (i + 1) * 2), 2), 2))]
    ...
    #[requires(src@.len() == dst@.len() * 4)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == signed_value(Self::decode(src@.subsequence(i * 4, (i + 1) * 4), 4), 4))]
    ...
    #[requires(src@.len() == dst@.len() * 8)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == signed_value(Self::decode(src@.subsequence(i * 8, (i + 1) * 8), 8), 8))]
    ...
    #[requires(src@.len() == dst@.len() * 16)]
    #[ensures(forall<i> 0 <= i && i < dst@.len() ==>
        (^dst)[i]@ == signed_value(Self::decode(src@.subsequence(i * 16, (i + 1) * 16), 16), 16))]
    ...
    /// assert_eq!(numbers_given, numbers_got);
    /// ```
    #[requires(dst@.len() == src@.len() * 2)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        Self::decode((^dst)@.subsequence(i * 2, (i + 1) * 2), 2) == src[i]@)]
    ...
    /// assert_eq!(numbers_given, numbers_got);
    /// ```
    #[requires(dst@.len() == src@.len() * 4)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        Self::decode((^dst)@.subsequence(i * 4, (i + 1) * 4), 4) == src[i]@)]
    ...
    /// assert_eq!(numbers_given, numbers_got);
    /// ```
    #[requires(dst@.len() == src@.len() * 8)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        Self::decode((^dst)@.subsequence(i * 8, (i + 1) * 8), 8) == src[i]@)]
    ...
    /// assert_eq!(numbers_given, numbers_got);
    /// ```
    #[requires(dst@.len() == src@.len() * 16)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        Self::decode((^dst)@.subsequence(i * 16, (i + 1) * 16), 16) == src[i]@)]
    ...
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        (^dst)[i]@ == if src[i]@ < 0 { src[i]@ + 256 } else { src[i]@ })]
    ...
    #[requires(dst@.len() == src@.len() * 2)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        signed_value(Self::decode((^dst)@.subsequence(i * 2, (i + 1) * 2), 2), 2) == src[i]@)]
    ...
    #[requires(dst@.len() == src@.len() * 4)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        signed_value(Self::decode((^dst)@.subsequence(i * 4, (i + 1) * 4), 4), 4) == src[i]@)]
    ...
    #[requires(dst@.len() == src@.len() * 8)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        signed_value(Self::decode((^dst)@.subsequence(i * 8, (i + 1) * 8), 8), 8) == src[i]@)]
    ...
    #[requires(dst@.len() == src@.len() * 16)]
    #[ensures(forall<i> 0 <= i && i < src@.len() ==>
        signed_value(Self::decode((^dst)@.subsequence(i * 16, (i + 1) * 16), 16), 16) == src[i]@)]
...
impl Invariant for BigEndian {
    /// `BigEndian` is an uninhabited marker type, so every possible value
    /// (there are none) satisfies its representation invariant.
    #[logic(open)]
...
impl DeepModel for LittleEndian {
    type DeepModelTy = u8;
...
impl Invariant for LittleEndian {
    /// `LittleEndian` is an uninhabited marker type, so every possible value
    /// (there are none) satisfies its representation invariant.
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { true }
...
macro_rules! read_slice {
    ($src:expr, $dst:expr, $ty:ty, $from_bytes:ident) => {{
        ...
        while i < dst.len() {
            let start = i * SIZE;
            dst[i] = <$ty>::$from_bytes(
                src[start..start + SIZE].try_into().unwrap(),
            );
            i += 1;
        }
    }};
...
macro_rules! write_slice {
    ($src:expr, $dst:expr, $ty:ty, $to_bytes:ident) => {{
        ...
        while i < src.len() {
            let start = i * SIZE;
            dst[start..start + SIZE].copy_from_slice(&src[i].$to_bytes());
            i += 1;
        }
    }};
}
...
impl ByteOrder for BigEndian {
    ...
    fn write_uint128(buf: &mut [u8], n: u128, nbytes: usize) {
        ...
        assert!(nbytes <= buf.len());
        let bytes = n.to_be_bytes();
        buf[..nbytes].copy_from_slice(&bytes[16 - nbytes..]);
    }
