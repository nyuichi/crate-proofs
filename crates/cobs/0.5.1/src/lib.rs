...
/// Mathematical maximum COBS overhead for a source length.
#[logic(open)]
pub fn encoding_overhead_model(source_len: Int) -> Int {
    pearlite! {
        if source_len == 0 { 1 } else { (source_len + 253) / 254 }
    }
}

/// Exact COBS encoding used by the public API contracts.
///
/// This symbol is intentionally opaque: clients reason through the algebraic
/// properties below without depending on the implementation's streaming
/// representation.
#[trusted]
#[logic(opaque)]
#[ensures(result.len() >= 1)]
#[ensures(result.len() <= source.len() + encoding_overhead_model(source.len()))]
#[ensures(forall<i: Int> 0 <= i && i < result.len() ==> result[i] != sentinel)]
pub fn cobs_encode_model(source: Seq<u8>, sentinel: u8) -> Seq<u8> {
    pearlite! { dead }
}

/// A canonical complete frame, with an optional trailing sentinel accepted by
/// the one-shot decoder.
#[logic(open)]
pub fn is_canonical_frame(input: Seq<u8>, plain: Seq<u8>, sentinel: u8) -> bool {
    pearlite! {
        input == cobs_encode_model(plain, sentinel)
            || input == cobs_encode_model(plain, sentinel).push_back(sentinel)
    }
}
...
#[inline]
#[ensures(result@ == encoding_overhead_model(source_len@))]
pub const fn max_encoding_overhead(source_len: usize) -> usize {
    if source_len == 0 {
        return 1;
    }
    #[cfg(creusot)]
    return (source_len - 1) / 254 + 1;
    #[cfg(not(creusot))]
...
#[inline]
#[requires(source_len@ + encoding_overhead_model(source_len@) <= usize::MAX@)]
#[ensures(result@ == source_len@ + encoding_overhead_model(source_len@))]
