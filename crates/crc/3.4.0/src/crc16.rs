...
{
    #[requires(valid_width_16(algorithm.width))]
    ...
    #[ensures(result == crc16_checksum_model(
    ...
    #[ensures(result == crc16_update_model(
        self.algorithm.width,
        self.algorithm.poly,
        self.algorithm.refin,
        crc,
    ...
    #[ensures(result.invariant())]
    #[ensures(result@ == crc16_init_model(
        self.algorithm.width,
        self.algorithm.refin,
        self.algorithm.init,
    ))]
    ...
    Table<L>: private::Sealed,
{
    ...
    #[ensures((^self)@ == crc16_update_model(
        self.algorithm().width,
        self.algorithm().poly,
        self.algorithm().refin,
        (*self)@,
    ...
    #[ensures(result == crc16_finalize_model(
        self.algorithm().width,
        self.algorithm().refin,
        self.algorithm().refout,
        self.algorithm().xorout,
...
#[trusted]
#[requires(valid_width_16(algorithm.width))]
#[ensures(result == crc16_init_model(algorithm.width, algorithm.refin, initial))]
...
/// slicing-by-16 block independently against `crc16_update_model`.
#[trusted]
#[requires(valid_width_16(algorithm.width))]
#[requires(crc16_table_valid(
    algorithm.width,
    algorithm.poly,
    algorithm.refin,
    *table,
