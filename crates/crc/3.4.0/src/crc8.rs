...
{
    #[requires(valid_width_8(algorithm.width))]
    ...
    #[ensures(result == crc8_checksum_model(
    ...
    #[ensures(result == crc8_update_model(
        self.algorithm.width,
        self.algorithm.poly,
        self.algorithm.refin,
        crc,
    ...
    #[ensures(result.invariant())]
    #[ensures(result@ == crc8_init_model(
        self.algorithm.width,
        self.algorithm.refin,
        self.algorithm.init,
    ))]
    ...
    Table<L>: private::Sealed,
{
    ...
    #[ensures((^self)@ == crc8_update_model(
        self.algorithm().width,
        self.algorithm().poly,
        self.algorithm().refin,
        (*self)@,
    ...
    #[ensures(result == crc8_finalize_model(
        self.algorithm().width,
        self.algorithm().refin,
        self.algorithm().refout,
        self.algorithm().xorout,
...
#[trusted]
#[requires(valid_width_8(algorithm.width))]
#[ensures(result == crc8_init_model(algorithm.width, algorithm.refin, initial))]
...
/// slicing-by-16 block independently against `crc8_update_model`.
#[trusted]
#[requires(valid_width_8(algorithm.width))]
#[requires(crc8_table_valid(
    algorithm.width,
    algorithm.poly,
    algorithm.refin,
    *table,
