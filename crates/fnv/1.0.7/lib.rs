...
use creusot_std::prelude::{
    ensures, invariant, logic, pearlite, proof_assert, snapshot, variant, Invariant, Seq, View,
};
...
impl View for FnvHasher {
    type ViewTy = u64;

    /// The current 64-bit FNV-1a state.
    ///
    /// This logical observer is intentionally opaque outside this crate so
    /// contracts can expose the hasher state without exposing its private
    /// representation.
    #[logic]
...
impl Invariant for FnvHasher {
    /// Every 64-bit bit pattern is a valid FNV-1a state, so there is no
    /// representation restriction for this invariant to impose. The meaning
    /// of the state is specified separately by the `View<u64>` implementation
    /// and the contracts expressed in terms of that view.
    #[logic(open)]
...
/// The state transition for one byte of an FNV-1a hash.
#[logic(open)]
#[ensures(result == (hash ^ byte as u64) * 0x100000001b3u64)]
pub fn fnv1a_step(hash: u64, byte: u8) -> u64 {
    pearlite! { (hash ^ byte as u64) * 0x100000001b3u64 }
}

/// The FNV-1a state obtained by processing `bytes` from `hash`.
#[logic]
#[variant(bytes.len())]
#[ensures(bytes.len() == 0 ==> result == hash)]
#[ensures(bytes.len() > 0 ==> result == fnv1a_step(
    fnv1a(hash, bytes.subsequence(0, bytes.len() - 1)),
    bytes[bytes.len() - 1]
))]
pub fn fnv1a(hash: u64, bytes: Seq<u8>) -> u64 {
    if bytes.len() == 0 {
        hash
    } else {
        fnv1a_step(
...
#[logic]
#[ensures(
    fnv1a(hash, bytes.push_back(byte)) == fnv1a_step(fnv1a(hash, bytes), byte)
)]
pub fn fnv1a_push(hash: u64, bytes: Seq<u8>, byte: u8) {
...
impl Default for FnvHasher {
    #[inline]
    #[ensures(result@ == 0xcbf29ce484222325u64)]
...
impl Hasher for FnvHasher {
    ...
    #[inline]
    #[ensures((^self)@ == fnv1a((*self)@, bytes@))]
    fn write(&mut self, bytes: &[u8]) {
        let FnvHasher(mut hash) = *self;
        let _initial_hash = snapshot! { hash };
        let mut index = 0usize;

        #[invariant(index@ <= bytes@.len())]
        #[invariant(hash == fnv1a(*_initial_hash, bytes@.subsequence(0, index@)))]
        while index < bytes.len() {
            let byte = bytes[index];
            let _hash_before = snapshot! { hash };
            hash = hash ^ (byte as u64);
            hash = hash.wrapping_mul(0x100000001b3);
            proof_assert!(hash == fnv1a_step(*_hash_before, byte));
            proof_assert!(
                bytes@.subsequence(0, index@ + 1)
                    == bytes@.subsequence(0, index@).push_back(byte)
            );
            proof_assert! {
                fnv1a_push(
                    *_initial_hash,
                    bytes@.subsequence(0, index@),
                    byte,
                );
                fnv1a(
                    *_initial_hash,
                    ...
                    fnv1a(*_initial_hash, bytes@.subsequence(0, index@)),
                    byte,
                )
            };
            index += 1;
