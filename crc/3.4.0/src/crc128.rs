...
{
    #[requires(valid_width_128(algorithm.width))]
    ...
    #[ensures(result == crc128_checksum_model(
    ...
    #[ensures(result == crc128_update_model(
        self.algorithm.width,
        self.algorithm.poly,
        self.algorithm.refin,
        crc,
    ...
    #[ensures(result.invariant())]
    #[ensures(result@ == crc128_init_model(
        self.algorithm.width,
        self.algorithm.refin,
        self.algorithm.init,
    ))]
    ...
    #[ensures(result@ == self.table_model())]
    ...
    Table<L>: private::Sealed,
{
    ...
    #[ensures((^self)@ == crc128_update_model(
        self.algorithm().width,
        self.algorithm().poly,
        self.algorithm().refin,
        (*self)@,
    ...
    #[ensures(result == crc128_finalize_model(
        self.algorithm().width,
        self.algorithm().refin,
        self.algorithm().refout,
        self.algorithm().xorout,
...
#[trusted]
#[requires(valid_width_128(algorithm.width))]
#[ensures(result == crc128_init_model(algorithm.width, algorithm.refin, initial))]
...
/// Trusted optimized-update boundary. TODO: prove the byte loop and the
/// slicing-by-16 block independently against `crc128_update_model`.
#[trusted]
#[requires(valid_width_128(algorithm.width))]
#[requires(crc128_table_valid(
    algorithm.width,
    algorithm.poly,
    algorithm.refin,
    *table,
