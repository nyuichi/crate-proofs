...
#[cfg_attr(not(creusot), derive(Clone))]
#[cfg_attr(all(not(feature = "nightly"), not(creusot)), derive(Default))]
#[cfg_attr(feature = "nightly", derive_const(Default))]
pub struct FxHasher {
    hash: usize,
}

impl View for FxHasher {
    type ViewTy = usize;

    /// The current machine-word polynomial-hash state.
    #[logic]
...
impl Invariant for FxHasher {
    /// Every machine-word bit pattern is a valid state.
    #[logic(open)]
...
#[cfg(all(creusot, not(feature = "nightly")))]
...
impl Hasher for FxHasher {
    ...
    #[ensures(exists<len_word: u64> len_word@ == bytes@.len()
        && (^self)@ == model::fx_write_u64(
            (*self)@,
            model::hash_bytes_model(bytes@, len_word),
        ))]
    ...
    #[inline]
    #[ensures((^self)@ == model::fx_step((*self)@, i as usize))]
    ...
    #[inline]
    #[ensures((^self)@ == model::fx_write_u64((*self)@, i))]
    ...
    #[inline]
    #[ensures((^self)@ == model::fx_write_u128((*self)@, i))]
    ...
    #[inline]
    #[ensures((^self)@ == model::fx_step((*self)@, i))]
    ...
    #[cfg(feature = "nightly")]
    #[inline]
    #[ensures(exists<len_word: u64> len_word@ == s@.to_bytes().len()
        && (^self)@ == model::fx_write_u64(
            (*self)@,
            model::hash_bytes_model(s@.to_bytes(), len_word),
        ))]
...
#[inline]
#[ensures(result == model::multiply_mix_model(x, y))]
...
impl Invariant for FxBuildHasher {
    #[logic(open)]
    fn invariant(self) -> bool {
        pearlite! { true }
    }
